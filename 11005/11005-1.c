...
int main()
{
    ...
    for (int i = 29; i >= 0; i--) // 최소인 2진수에서 29제곱부터 가능하므로 높은 제곱부터 차례로 해줘야함 (낮은 제곱부터 하면 결정 불가)
    {
        if (pow(notation, i) <= 1e9)
            pow_ = pow(notation, i);
        else
            continue;

        if (number - pow_ >= 0) // B진수의 해당 자리가 존재하는지 계산 ex) i = 29, B = 2일 때, 2진수의 29자릿 수가 존재하는지 계산
        {
